 ...
 *
 * Este gamemode é voltado para diversão casual com funcionalidades básicas.
 * Contém estrutura de inicialização, carregamento e configuração principal.
 *
 * @author Vico
 * @license MIT
 * @version 0.1b
...
/* ---------------------------- VARIÁVEIS GLOBAIS ---------------------------- */
// TEMP: RETIRAR DEPOIS!
new sweet = INVALID_VEHICLE_ID;

// Cores pro textdraw de carro
new RandomColors[][] =
{
    "~r~", "~w~", "~g~", "~b~", "~y~", "~p~",
    "~r~~h~", "~g~~h~", "~y~~h~", "~p~~h~", "~b~~h~",
...
// Nomes dos veículos pro textdraw de carro
new VehicleNames[][] =
{
...
public OnGameModeInit()
{
    ...
    // TEMP: RETIRAR DEPOIS!
    sweet = CreateVehicle(492, 2508.3970,-1666.3979,13.1808,11.5191, 2, 1, 60000);
    SetVehicleNumberPlate(sweet, "GROVE4L");
    SetVehicleToRespawn(sweet);
...
{
    // TEXTDRAW DE CARRO (quando o jogador entrar em um ele mostra o nome do veiculo)
    new string[22];
    if(newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)
    {
        format(string, 22, "%s%s", RandomColors[random(sizeof(RandomColors))], VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
